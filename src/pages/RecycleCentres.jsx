...
const capacityPct = (centre) =>
  centre.maxCapacityKg > 0
    ? Math.round((centre.currentLoadKg / centre.maxCapacityKg) * 100)
    : 0;
const capColor = (pct) =>
  pct >= 90
    ? "var(--danger)"
...
function validateCentre(payload, validWasteTypes) {
  ...
  if (payload.acceptedWasteTypes.length === 0)
  ...
  if (!Number.isFinite(payload.maxCapacityKg) || payload.maxCapacityKg < 1)
    return "Max capacity must be at least 1 kg.";
  if (!Number.isFinite(payload.currentLoadKg) || payload.currentLoadKg < 0)
    return "Current load must be 0 or greater.";
  if (payload.currentLoadKg > payload.maxCapacityKg)
    return "Current load cannot exceed max capacity.";
...
export default function RecycleCentres() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const message =
        ...
        "Failed to load recycle centres.";
      setCentres([]);
      setPageErr(message);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setCentres(r.data.recyclingCenters || []);
      showToast(
        `Found ${r.data.recyclingCenters?.length || 0} recycle centre${(r.data.recyclingCenters?.length || 0) !== 1 ? "s" : ""}.`,
      );
    } catch (e) {
      const message =
        e.response?.data?.message ||
  ...
  const handleFilterWaste = async (wasteType) => {
    ...
    try {
      ...
      setCentres(r.data.recyclingCenters || []);
      showToast(
        `Showing ${r.data.recyclingCenters?.length || 0} centre${(r.data.recyclingCenters?.length || 0) !== 1 ? "s" : ""} for ${wasteType}.`,
      );
    } catch (e) {
      const message =
        e.response?.data?.message ||
        e.response?.data?.error ||
        "Failed to filter recycle centres.";
      setCentres([]);
      setPageErr(message);
      showToast(message, "error");
    } finally {
    ...
    try {
      const payload = {
        ...
        location: {
          type: "Point",
          coordinates: [
            Number(cForm.location.coordinates[0]),
            Number(cForm.location.coordinates[1]),
          ],
        },
      ...
      showToast(
        cModal === "new"
          ? "Recycle centre created successfully."
          : "Recycle centre updated successfully.",
      );
      load();
    } catch (e) {
      const message =
        e.response?.data?.message || e.response?.data?.error || "Error";
      setErr(message);
      showToast(message, "error");
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const message =
        e.response?.data?.message ||
        e.response?.data?.error ||
        "Failed to delete recycle centre.";
      setPageErr(message);
      showToast(message, "error");
    } finally {
      setDeletingId("");
  ...
  return (
    <div>
      ...
      <div
        className="flex-between page-header"
        style={{ flexWrap: "wrap", gap: "1rem" }}
      >
        <div>
          <h1 className="page-title">Recycle Centres</h1>
          <p className="page-subtitle">
            Find nearby recycling drop-off locations
          </p>
        </div>
        {isAdmin && (
      ...
      >
        ...
        <select
          ...
          style={{ width: 220 }}
          value={filterWaste}
          onChange={(e) => handleFilterWaste(e.target.value)}
        >
      ...
      {loading ? (
        ...
        <div className="grid-2">
          {centres.map((centre) => {
            ...
            const centreWasteTypes = uniqueWasteTypes(
              centre.acceptedWasteTypes,
            );
            ...
            return (
              <div
                ...
                onClick={() =>
                  setSelected((current) =>
                    getCentreId(current) === centreId ? null : centre,
                  )
                }
              >
                <div
                ...
                >
                  ...
                  <span
                    className="badge"
                    style={{
                      background: `${capColor(pct)}22`,
                      color: capColor(pct),
                      border: `1px solid ${capColor(pct)}44`,
                    }}
                  >
                    {pct}% full
                  </span>
                ...
                >
                  {centreWasteTypes.map((wasteType, index) => (
                    <span
                      key={`${centreId || centre.name}-${wasteType}-${index}`}
                ...
                <div>
                  ...
                  >
                    <span>Capacity</span>
                    <span>
                      {centre.currentLoadKg?.toLocaleString()} /{" "}
                      {centre.maxCapacityKg?.toLocaleString()} kg
                    </span>
                  </div>
                  <div className="progress-bar">
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        background: `linear-gradient(90deg, ${capColor(pct)}, ${capColor(pct)}aa)`,
                      }}
                    />
                  </div>
                </div>
                    ...
                    {isAdmin && (
                      <button
                        ...
                        onClick={() =>
                          setConfirmState({
                            id: centreId,
                            name: centre.name || "this recycle centre",
                          })
                        }
                      >
                        Delete
      ...
      {cModal !== null && (
        <Modal
          title={cModal === "new" ? "Add Recycling Centre" : "Edit Centre"}
        ...
        >
          ...
          {wasteTypes.length === 0 && (
            <div className="error-msg" style={{ marginBottom: "1rem" }}>
              No waste categories are available. Add categories first before
              creating or updating a recycle centre.
            </div>
          )}
          ...
          >
            <div className="form-group">
              ...
              <input
                ...
                value={cForm.name}
                onChange={(e) =>
                  setCForm((form) => ({ ...form, name: e.target.value }))
                }
              />
            </div>
            ...
            <div className="form-group">
              ...
              <input
                ...
                value={cForm.address}
                onChange={(e) =>
                  setCForm((form) => ({ ...form, address: e.target.value }))
                }
              />
            </div>
            ...
            <div className="form-group">
              ...
              >
                {wasteTypes.map((wasteType) => (
                  ...
                  >
                    {cForm.acceptedWasteTypes.includes(wasteType)
                      ? "Selected: "
                      : ""}
                    {wasteType}
                  </button>
            ...
            <button
              className="btn btn-primary"
              onClick={saveCentre}
              disabled={saving || wasteTypes.length === 0}
            >
              {saving ? (
